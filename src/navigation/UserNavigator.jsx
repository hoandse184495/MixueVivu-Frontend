...
function TabIcon({ name, focused }) {
    ...
    return (<View style={[styles.tabIconWrapper, focused && styles.tabIconWrapperActive]}>
...
export default function UserNavigator({ onLogout }) {
    const { width } = useWindowDimensions();
    const isLargeScreen = width >= 1024;
    return (<Tab.Navigator tabBar={(props) => (<ResponsiveTabBar {...props} onLogout={onLogout} role="user" primaryColor={COLORS.primary} primaryColorLight={COLORS.primaryLight}/>)} screenOptions={{
            headerShown: false,
            sceneStyle: {
      ...
      <Tab.Screen name="HomeTab" options={{
            tabBarIcon: ({ focused }) => (<TabIcon name="Trang chủ" focused={focused}/>),
        }}>
        {() => <HomeStack onLogout={onLogout}/>}
      </Tab.Screen>

      <Tab.Screen name="FavoritesTab" component={FavoriteStack} options={{
            tabBarIcon: ({ focused }) => (<TabIcon name="Yêu thích" focused={focused}/>),
        }}/>

      <Tab.Screen name="BookingsTab" component={BookingStack} options={{
            tabBarIcon: ({ focused }) => (<TabIcon name="Booking" focused={focused}/>),
        }}/>

      <Tab.Screen name="PaymentsTab" component={UserPaymentsScreen} options={{
            tabBarIcon: ({ focused }) => (<TabIcon name="Thanh toán" focused={focused}/>),
        }}/>

      <Tab.Screen name="FriendsTab" component={FriendScreen} options={{
            tabBarIcon: ({ focused }) => (<TabIcon name="Bạn bè" focused={focused}/>),
        }}/>

      <Tab.Screen name="ProfileTab" options={{
            tabBarIcon: ({ focused }) => (<TabIcon name="Hồ sơ" focused={focused}/>),
        }}>
