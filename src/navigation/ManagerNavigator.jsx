...
export default function ManagerNavigator({ onLogout, }) {
    ...
    return (<Tab.Navigator tabBar={(props) => (<ResponsiveTabBar {...props} onLogout={onLogout} role="manager" primaryColor={COLORS.primary} primaryColorLight="#e8f0fe"/>)} screenOptions={{
            headerShown: false,
            sceneStyle: {
                marginLeft: isLargeScreen ? 260 : 0,
            },
        }}>
      <Tab.Screen name="ManagerDashboardTab" options={{
            tabBarIcon: ({ focused }) => (<TabIcon name="Dashboard" focused={focused}/>),
        }}>
        {() => <DashboardStack onLogout={onLogout}/>}
      </Tab.Screen>

      <Tab.Screen name="PendingToursTab" component={ManagerPendingToursScreen} options={{
            tabBarIcon: ({ focused }) => (<TabIcon name="Pending Tours" focused={focused}/>),
        }}/>

      <Tab.Screen name="ManagerBookingsTab" component={ManagerBookingsScreen} options={{
            tabBarIcon: ({ focused }) => (<TabIcon name="Bookings" focused={focused}/>),
        }}/>

      <Tab.Screen name="UsersTab" component={ManagerUsersScreen} options={{
            tabBarIcon: ({ focused }) => (<TabIcon name="Users" focused={focused}/>),
        }}/>

      <Tab.Screen name="PaymentsTab" component={ManagerPaymentsScreen} options={{
            tabBarIcon: ({ focused }) => (<TabIcon name="Payments" focused={focused}/>),
        }}/>

      <Tab.Screen name="PayoutsTab" component={ManagerPayoutsScreen} options={{
