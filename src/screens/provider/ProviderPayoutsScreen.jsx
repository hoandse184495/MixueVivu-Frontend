...
import { bookingService, payoutService } from '../../services';
...
export default function ProviderPayoutsScreen() {
    ...
    const revenueSummary = useMemo(() => revenueBookings.reduce((summary, booking) => ({
    ...
    const renderPayoutCard = ({ item }) => {
        ...
        <View style={styles.cardBody}>
          ...
          {(payout === null || payout === void 0 ? void 0 : payout.paidAt) && (<View style={styles.row}>
    ...
    return (<SafeAreaView style={[styles.container, { backgroundColor: colors.bg }]}>
      ...
      {loading ? (<ActivityIndicator size="large" color={COLORS.primary} style={{ marginTop: 40 }}/>) : (<FlatList data={revenueBookings} keyExtractor={(item) => item.id.toString()} renderItem={renderPayoutCard} contentContainerStyle={styles.listContent} onRefresh={fetchRevenueData} refreshing={loading} ListHeaderComponent={renderListHeader} ListEmptyComponent={<View style={styles.emptyContainer}>
