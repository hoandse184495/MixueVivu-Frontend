...
export default function BookingScreen({ navigation, route }: Props) {
  ...
  const handleBook = async () => {
    ...
    Alert.alert(
      'Xác nhận đặt tour',
      `Đặt tour "${tour.title}" cho ${numPeople} người với tổng tiền ${totalPrice.toLocaleString('vi-VN')}₫?`,
      [
        { text: 'Hủy', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('🎉 Đặt tour thành công!', 'Booking của bạn đang được xử lý.', [
                {
                  text: 'OK',
              ...
              Alert.alert(
                'Lỗi',
                e.response?.data?.message || 'Không thể đặt tour'
              );
            } finally {
              setLoading(false);
            }
        ...
        <View style={styles.section}>
          ...
          <View style={styles.counterCard}>
            ...
            <View style={styles.counterControls}>
              ...
              <TouchableOpacity
                style={[styles.counterBtn, styles.counterBtnAdd, numPeople >= tour.availableSlots && styles.counterBtnDisabled]}
                onPress={() => setNumPeople(Math.min(tour.availableSlots, numPeople + 1))}
                disabled={numPeople >= tour.availableSlots}
              >
          ...
          <View style={styles.infoList}>
            ...
            {tour.guideName && (
              <>
                <View style={styles.infoDivider} />
                <View style={styles.infoRow}>
                  <Text style={styles.infoLabel}>Hướng dẫn viên</Text>
                  <Text style={styles.infoValue}>{tour.guideName}</Text>
                </View>
              </>
            )}
          </View>
