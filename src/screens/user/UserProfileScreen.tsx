...
export default function UserProfileScreen({ navigation, onLogout }: Props) {
  ...
  const menuItems = [
    ...
    { icon: '🎫', label: 'Booking của tôi', color: colors.primaryLight, textColor: colors.primary, action: () => navigation.getParent()?.navigate('BookingsTab') },
    { icon: '❤️', label: 'Tour yêu thích', color: colors.surfaceContainerLow, textColor: colors.secondary, action: () => navigation.getParent()?.navigate('FavoritesTab') },
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.bg }]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <View style={[styles.profileHero, { backgroundColor: colors.surface, borderBottomColor: `${colors.border}40` }]}>
          ...
          <Text style={[styles.profileName, { color: colors.text }]}>{user?.fullName || 'Người dùng'}</Text>
          ...
          <View style={styles.modalOverlay}>
            <View style={[styles.modalCard, { backgroundColor: colors.surface }]}>
              ...
              <TextInput
                ...
                value={form.avatar}
                onChangeText={(avatar) => setForm((current) => ({ ...current, avatar }))}
                placeholder="https://..."
                placeholderTextColor={colors.textMuted}
                autoCapitalize="none"
              />
              ...
              <View style={styles.modalActions}>
                <TouchableOpacity
                  style={[styles.modalBtn, styles.cancelBtn, { backgroundColor: colors.surfaceContainerLow }]}
                ...
                <TouchableOpacity
                  style={[styles.modalBtn, styles.saveBtn, { backgroundColor: colors.primary }, savingProfile && styles.saveBtnDisabled]}
