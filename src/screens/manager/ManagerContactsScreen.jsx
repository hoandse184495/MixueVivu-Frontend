...
export default function ManagerContactsScreen() {
    ...
    const fetchContacts = useCallback(async () => {
        ...
        try {
            ...
            Alert.alert('Lỗi', ((_b = (_a = error.response) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.message) || 'Không thể tải danh sách liên hệ');
        }
        finally {
            setLoading(false);
        }
    ...
    const handleSendReply = async () => {
        ...
        try {
            ...
            Alert.alert('Lỗi', ((_b = (_a = error.response) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.message) || 'Lỗi khi gửi phản hồi');
        }
    };
    const renderContactCard = ({ item }) => {
        ...
        {isReplied ? (<View style={styles.replyContainer}>
            <Text style={styles.replyTitle}>Đã phản hồi:</Text>
            <Text style={styles.replyText}>{item.reply}</Text>
          </View>) : (<TouchableOpacity style={styles.replyBtn} onPress={() => handleOpenReply(item)} activeOpacity={0.8}>
    ...
    return (<SafeAreaView style={styles.safeArea}>
      ...
      {loading && contacts.length === 0 ? (<View style={styles.loadingContainer}>
          <ActivityIndicator size="large" color={COLORS.primary}/>
          <Text style={styles.loadingText}>Đang tải danh sách liên hệ...</Text>
        </View>) : (<FlatList data={contacts} keyExtractor={(item) => item.id.toString()} renderItem={renderContactCard} contentContainerStyle={styles.listContent} showsVerticalScrollIndicator={false} onRefresh={fetchContacts} refreshing={loading} ListEmptyComponent={<View style={styles.emptyContainer}>
      ...
      <Modal visible={replyModalVisible} transparent={true} animationType="fade" onRequestClose={() => setReplyModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            {selectedContact && (<View style={styles.modalContactSummary}>
            ...
            <TextInput style={styles.replyInput} placeholder="Nhập nội dung phản hồi chi tiết cho khách hàng..." placeholderTextColor={COLORS.textMuted} multiline={true} numberOfLines={5} value={replyText} onChangeText={setReplyText}/>

            <View style={styles.modalButtons}>
              <TouchableOpacity style={styles.modalCancelBtn} onPress={() => setReplyModalVisible(false)}>
